...
import time  # 時間待機ライブラリ（クッキーの保存完了を待つために使用）
...
# ==========================================
controller = CookieController()  # ブラウザのクッキーを読み書きするためのインスタンス

if "user" not in st.session_state:
    st.session_state.user = None

# 【安全な自動ログイン】クッキーが届く前のエラーを try-except で回避
if st.session_state.user is None:
    try:
        ...
        if access_token and refresh_token:
            ...
            st.rerun()  # ログインに成功したら画面をリロードしてメイン画面へ
    except Exception:
        # まだクッキーが届いていない（準備不足）時のエラーは無視してやり過ごす
        pass
...
# 【画面完全分離ブロック】未ログインならここでストップ！
if st.session_state.user is None:
    login_ui()
    st.stop()  # ログインが済んでいない場合は、これより下のプログラムは一切実行されません
